...
func loadConfig(path string, data map[string]interface{}) (*Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("WriteBuffer %v: %v", string(b), err)
	}
	...
	if filepath.Ext(path) == ".yaml" {
		...
		if err != nil {
			return nil, fmt.Errorf("YAMLToJSON %v: %v", string(cj), err)
		}
	...
	if err := hclsimple.Decode(path, cj, nil, c); err != nil {
		return nil, fmt.Errorf("decode %v: %v", string(cj), err)
	}
	if err := c.Init(); err != nil {
		return nil, fmt.Errorf("init %v: %v", c, err)
	}
	return c, nil
}

...
variable "parent_id" {
  type        = string
  description = "ID of parent GCP resource to apply the policy. Can be one of the organization ID or folder ID according to parent_type."
  validation {
    condition     = can(regex("^[0-9]{8,25}$", var.parent_id))
    error_message = "The parent_id must be valid. Should have only numeric values with a length between 8 and 25 digits."
  }
}
...
variable "project" {
  ...
  validation {
    ...
    error_message = "Invalid project.project_id."
  }
}

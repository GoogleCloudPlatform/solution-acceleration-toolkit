...
To run against the current working directory:
...
To run on a different directory:
...
By default, the importer will call `terraform` to do operations. This may be a
binary on the PATH or an alias.

To use a specific terraform binary, or a wrapper like
[Terragrunt](https://terragrunt.gruntwork.io/), supply the right flag:

```shell
tfimport --terraform_path='terragrunt'
```
